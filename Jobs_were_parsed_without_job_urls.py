...
cur.execute("select ra.company_id, c.company, case when ra.agent_id = 0 then 'cd' when ra.agent_id = 1 then 'ats' when ra.agent_id = 3 then 'table parser' when ra.agent_id = 9 then 'aggs' end as parsing_agent, ra.job_id,	ra.external_ref_id, ra.url, ra.location	from vcs_audit.raw_audit ra inner join vcs.companies c on ra.company_id = c.id where (ra.url is null or ra.url = '' or ra.url = ' ') and ra.tstamp > subdate(now(), interval 1 day) group by ra.company_id;")
...
df = pd.DataFrame(dic)
df_csv = df.to_csv('Jobs_were_parsed_without_job_urls.csv',sep=';',index=False)
...
msg = MIMEMultipart()
msg['Subject'] = 'ALERT - Jobs were parsed without job urls  ' + str(today)
msg['From'] = sender_email
msg['To'] = ','.join(receiver_email)

part = MIMEBase('application', 'octet-steam')
part.set_payload(open('Jobs_were_parsed_without_job_urls.csv','rb').read())
encoders.encode_base64(part)
part.add_header('Content-Disposition','attachment; filename = "Jobs_were_parsed_without_job_urls.csv"')
body = "Invalid Combination - Jobs were parsed without job urls"
